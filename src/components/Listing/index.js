...
function Listing(props) {
    ...
    const isDisabledNextBtn = props.page * props.page >= props.count;
        ...
        props.items.length ? <div className="listing">
            <div className="listing__pagination">
                <button className={`${btnBaseClass} ${btnBaseClass}--prev ${isDisabledPrevBtn && `${btnBaseClass}--disabled`}`}
                    ...
                    />
                <span
                    className="listing__count">page {props.page} / {Math.ceil(props.count / props.perPage)}</span>
                <button
                    className={`${btnBaseClass} ${btnBaseClass}--next ${isDisabledNextBtn && `${btnBaseClass}--disabled`}`}
